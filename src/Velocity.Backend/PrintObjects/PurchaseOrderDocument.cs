...
{
    private readonly PurchaseOrderResponse _purchaseOrderResponse;
    private readonly SupplierResponse _supplierResponse;

    private PurchaseOrderDocument(PurchaseOrderResponse purchaseOrderResponse, SupplierResponse supplierResponse)
    {
        _purchaseOrderResponse = purchaseOrderResponse;
        _supplierResponse = supplierResponse;
    }
    
    public static byte[] Generate(PurchaseOrderResponse purchaseOrderResponse, SupplierResponse supplierResponse)
    {
        return new PurchaseOrderDocument(purchaseOrderResponse, supplierResponse).GeneratePdf();
    }
    ...
    public void Compose(IDocumentContainer container)
    {
        container.Page(page =>
        {
            ...
            {
                column.Spacing(5);
                column.Item().Text("This is a computer generated document. No signature is required.").FontSize(8f);
                column.Item().Row(row =>
                {
    ...
    private void ComposeHeader(IContainer container)
    {
        ...
        container.Row(row =>
        {
            ...
            {
                column.Item().Text($"Order Number: {_purchaseOrderResponse.OrderNumber}").Style(titleStyle);

                column.Item().Text(text =>
                {
                    text.Span("Order date: ").SemiBold();
                    text.Span($"{_purchaseOrderResponse.OrderDate:dd/MM/yyyy}");
                });
                
                column.Item().Text(text =>
                {
                    text.Span("Supplier Reference Number: ").SemiBold();
                    text.Span($"{_purchaseOrderResponse.SupplierReferenceNumber}");
                });
    ...
    private void ComposeContent(IContainer container)
    {
        container.PaddingVertical(10).Column(column =>
        {
            ...
            column.Item().AlignRight().Text($"Grand total: {_purchaseOrderResponse.Total:C}").FontSize(13f);
        });
    }
